...
from chesscog.corner_detection.visualize import draw_lines


def find_corners(cfg, img: np.ndarray) -> np.ndarray:
    ...
    iterations = 0
    while iterations < 1000 or best_num_inliers < 30:
        ...
        if num_inliers > best_num_inliers:
            ...
            (xmin, xmax, ymin,
             ymax), scale, quantized_points, intersection_points, warped_img_size = configuration = _quantize_points(
...
def resize_image(cfg, img: np.ndarray) -> typing.Tuple[np.ndarray, float]:
...
def _detect_edges(edge_detection_cfg, gray: np.ndarray) -> np.ndarray:
...
def _detect_lines(cfg, edges: np.ndarray) -> np.ndarray:
    # array of [rho, theta]
...
def get_intersection_point(rho1: np.ndarray, theta1: np.ndarray, rho2: np.ndarray, theta2: np.ndarray) -> typing.Tuple[
...
def _find_best_scale(cfg, values: np.ndarray, scales: np.ndarray = np.arange(1, 9)):
...
def _discard_outliers(cfg, warped_points: np.ndarray, intersection_points: np.ndarray) -> typing.Tuple[
...
def _quantize_points(cfg, warped_scaled_points: np.ndarray, intersection_points: np.ndarray) -> typing.Tuple[
...
def _compute_vertical_borders(cfg, warped: np.ndarray, mask: np.ndarray, scale: np.ndarray, xmin: int, xmax: int) -> \
...
def _compute_horizontal_borders(cfg, warped: np.ndarray, mask: np.ndarray, scale: np.ndarray, ymin: int, ymax: int) -> \
typing.Tuple[int, int]:
